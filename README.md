---
title: Jarbas RAG (Projeto de Estudo Pessoal)
emoji: 🧑‍🏫
colorFrom: blue
colorTo: purple
sdk: streamlit
...
> Você pode gerar respostas **com OpenAI** (janela grande e mais qualidade) **ou 100% local** (modelo leve TinyLlama-1.1B-Chat-v1.0), sempre **nos seus próprios dados** indexados com FAISS.
...
│     ├─ rag/
│     │  ├─ local_backend.py   # backend RAG LOCAL (TinyLlama-1.1B-Chat-v1.0)
...
| Aspecto             | OpenAI (gpt-4o-mini)                      | Local (TinyLlama-1.1B-Chat-v1.0)                              |
|---------------------|-------------------------------------------|-----------------------------------------------|
...
- **Local (`src/jarbas/rag/local_backend.py`)**  
  Usa `TinyLlama/TinyLlama-1.1B-Chat-v1.0` (via `transformers`/`pipeline` com `text-generation`).  
...
- **Local (TinyLlama-1.1B-Chat-v1.0)**: parâmetros **travados** na UI para evitar `Killed`/OOM.
...
1. **Escolha do motor**
   ...
   - **Local (TinyLlama-1.1B-Chat-v1.0)** — não exige chave; **parâmetros travados** por segurança.
...
O TinyLlama-1.1B-Chat-v1.0 tem **janela menor**. O backend aplica **truncamentos**. Tente encurtar a pergunta ou use o motor **OpenAI**.
