...
> Você pode gerar respostas **com OpenAI** (janela grande e mais qualidade) **ou 100% local** (modelo leve Qwen2.5- 1.5B-Instruct), sempre **nos seus próprios dados** indexados com FAISS.
...
│     ├─ rag/
│     │  ├─ local_backend.py   # backend RAG LOCAL (Qwen2.5-1.5B-Instruct)
...
| Aspecto             | OpenAI (gpt-4o-mini)                      | Local (Qwen2.5-1.5B-Instruct)                              |
|---------------------|-------------------------------------------|-----------------------------------------------|
...
- **Local (Qwen2.5-1.5B)**: parâmetros **travados** na UI para evitar `Killed`/OOM.
...
1. **Escolha do motor**
   ...
   - **Local (Qwen2.5-1.5B)** — não exige chave; **parâmetros travados** por segurança.
...
O Qwen2.5- 1.5B tem **janela menor**. O backend aplica **truncamentos**. Tente encurtar a pergunta ou use o motor **OpenAI**.
